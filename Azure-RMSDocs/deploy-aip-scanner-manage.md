...
すべてのファイルは、スキャナーで自動および推奨のラベル設定が新しく設定または変更されたときにも検査されます。 スキャナーは、4時間ごとにポリシーを自動的に更新します。

テスト中など、ポリシーをすぐに更新するには、 **%LocalAppData%\Microsoft\MSIP\mip \\ < *processname*> \ mip**ディレクトリの内容を手動で削除し、Azure Information Protection サービスを再起動します。
