...
RMS コネクタでは、次のオンプレミスサーバーがサポートされています。Exchange Server、SharePoint Server、および、Windows Server を実行していて、ファイル分類インフラストラクチャを使ってフォルダー内の Office ドキュメントを分類してポリシーを適用しているファイル サーバーです。 
