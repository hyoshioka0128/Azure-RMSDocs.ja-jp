...
3. **Azure Rights Management 検出サービス**: RMS クライアントは **`https://discover.aadrm.com`** に接続し、ユーザーに認証を求めます。
    ...
    *\<the No__t Anturl @* aadrm の形式は次のとおりです: **{GUID}. Rms. [Region].** 。この値は、 [AipServiceConfiguration](/powershell/module/aipservice/get-aipserviceconfiguration)コマンドレットを実行したときに**RightsManagementServiceId**値を識別することによって確認できます。
