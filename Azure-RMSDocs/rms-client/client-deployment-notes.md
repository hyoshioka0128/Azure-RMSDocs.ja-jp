...
|                                                                                    **トレースを有効化または無効化するには**                                                                                    |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    次のレジストリ キーを更新します。<br /><br />HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\MSIPC<br />REG_DWORD:Trace<br /><br />**値:** トレースを有効にする場合は1、トレースを無効にする場合は 0 (既定値)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
1. **ローカル コンピューターの Windows レジストリ**: サービス検出の設定がレジストリで構成されている場合は、これらの設定が最初に試行されます。 
...
2. **Active Directory Domain Services**: ドメインに参加しているコンピューターは、サービス接続ポイント (SCP) の Active Directory に対してクエリを行います。 
...
3. **Azure Rights Management 検出サービス**: RMS クライアントは **`https://discover.aadrm.com`** に接続し、ユーザーに認証を求めます。
    ...
    例えば:5c6bb73b-1038-4eec-863d-49bded473437.rms.na.aadrm.com/_wmcs/licensing

    ここでは、" **{GUID}. rms. [Region]. aadrm**" という形式になっています。 *\<\>* この値は、 [AipServiceConfiguration](/powershell/module/aipservice/get-aipserviceconfiguration)コマンドレットを実行したときに**RightsManagementServiceId**値を識別することによって確認できます。
...
2.  レジストリ エディターで、次のいずれかに移動します。
    ...
    -   X64 プラットフォーム上の64ビットバージョンの Office の場合:HKLM\SOFTWARE\Microsoft\MSIPC\Servicelocation

    -   X64 プラットフォーム上の32ビットバージョンの Office の場合:HKLM\SOFTWARE\Wow6432Node\Microsoft\MSIPC\Servicelocation
