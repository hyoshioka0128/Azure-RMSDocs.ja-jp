...
2. *OnBeHalfOf*パラメーターを指定して、Set-AIPAuthentication コマンドレットを実行します。これには、作成した変数の値を指定します。 また、アプリの登録値、テナント ID、および Azure AD で委任されたユーザーアカウントの名前を指定します。 例 :
...
PowerShell セッションでコマンドレットのヘルプを表示するには、「`Get-Help <cmdlet name> -online`」と入力します。 例 : 
