---
title: "Azure Information Protection クライアントでサポートされるファイルの種類 | Azure Information Protection"
...
### <a name="supported-file-types-for-protection-and-their-file-name-extensions"></a>保護に対してサポートされるファイルの種類と、そのファイル名拡張子
Azure Information Protection クライアントによってネイティブにサポートされているファイルの種類の一覧を、次の表に示します。 これらのファイルの種類については、ネイティブの保護が適用され、これらのファイルが読み取り専用になると、元のファイル名拡張子が変更されます。
...
Azure Information Protection クライアントによってネイティブにサポートされる Microsoft Office アプリのファイルの種類の一覧を、次の表に示します。 これらのファイルの場合、ファイル名拡張子は、ファイルが Rights Management サービスで保護された後も変更されません。
